...
export class NewCollectionModal {
  ...
  constructor(page: Page) {
    ...
    this.nameInput = page
      .locator(
        '[data-test-id="collection-name-input"], input[name="name"], #name, input[placeholder*="nazwa"], textarea[placeholder*="nazwa"]'
      )
      .first();
    ...
    this.descriptionInput = page
      .locator(
        '[data-test-id="collection-description-input"], textarea[name="description"], #description, textarea[placeholder*="opis"], textarea'
      )
      .first();
    ...
    this.createButton = page
      .locator(
        '[data-test-id="create-collection-button"], button:has-text("Utwórz"), button:has-text("Zapisz"), button[type="submit"]'
      )
      .first();

    // Szukamy przycisku anulowania
    this.cancelButton = page
      .locator('[data-test-id="cancel-button"], button:has-text("Anuluj"), button[type="button"]:has-text("Anuluj")')
      .first();

    // Szukamy komunikatu sukcesu
    this.successMessage = page
      .locator(
        '[data-test-id="success-message"], .alert-success, .success, div:has-text("utworzona pomyślnie"), div:has-text("utworzono kolekcję"), div:has-text("zapisano kolekcję"), div.success'
      )
      .first();
    ...
    this.errorMessage = page
      .locator(
        '[data-test-id="error-message"], .alert-danger, .error, div:has-text("błąd"), div:has-text("wymagana"), div.error'
      )
      .first();
  }
  ...
  async verifySuccess(name?: string) {
    ...
    if (name) {
      const newCollectionVisible = await this.page
