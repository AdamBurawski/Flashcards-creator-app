...
export class CollectionsPage {
  ...
  constructor(page: Page) {
    ...
    this.collectionItems = page.locator('.collection-item, .card, div[role="listitem"], div:has-text("Kolekcja")').first().locator('..');
    
    // Szukamy nazw kolekcji na podstawie struktury widocznej na ekranie
    this.collectionNames = page.locator('h2, h3, .collection-name, .card-title, div:has-text("Kolekcja")').first().locator('..');
  }
  ...
  async clickNewCollection() {
    ...
    const createButton = this.page.locator('button:has-text("Nowa kolekcja"), a:has-text("Nowa kolekcja"), button:has-text("+"), [data-test-id="new-collection-button"]');
