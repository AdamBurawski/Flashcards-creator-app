...
test.describe("Import fiszek", () => {
  test("powinien umożliwić import fiszek do kolekcji", async ({
    ...
    const generationSelectEnabled = await importDialog.generationSelect.isEnabled().catch(() => false);
    ...
    if (generationSelectEnabled) {
      ...
      const hasFlashcards = await importDialog.flashcardsList.isVisible().catch(() => false);
