...
export const GET: APIRoute = async ({ params, locals }) => {
  try {
    ...
    if (!idParam) {
      return new Response(JSON.stringify({ error: "Brak ID kolekcji" }), {
        status: 400,
    ...
    if (isNaN(collectionId)) {
      return new Response(JSON.stringify({ error: "Nieprawidłowe ID kolekcji" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (!isBypassMode) {
      ...
      if (!hasAccess) {
        ...
        return new Response(JSON.stringify({ error: error || "Brak dostępu do kolekcji" }), {
          status: statusCode,
          headers: { "Content-Type": "application/json" },
        });
      }
    ...
    return new Response(JSON.stringify({ error: "Wystąpił nieoczekiwany błąd podczas przetwarzania żądania" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
