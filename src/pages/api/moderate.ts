...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!apiKey) {
    ...
    return new Response(JSON.stringify({ error: "Klucz API (OPENAI_API_KEY) nie jest skonfigurowany." }), {
      status: 500,
  ...
  try {
    ...
    if (typeof input !== "string" || input.trim() === "") {
      console.error('Błąd walidacji: Pole "input" jest wymagane i musi być niepustym ciągiem znaków.', {
        receivedInput: input,
      });
      return new Response(
        JSON.stringify({
          error: 'Nieprawidłowe dane wejściowe. Pole "input" jest wymagane i musi być niepustym ciągiem znaków.',
        }),
    ...
    return new Response(
      JSON.stringify({
        error: 'Nie udało się przetworzyć żądania. Upewnij się, że wysyłasz poprawny JSON z polem "input".',
      }),
      { status: 400, headers: { "Content-Type": "application/json" } }
    );
  }
