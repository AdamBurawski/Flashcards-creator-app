...
interface NewCollectionDialogProps {
  onClose: () => void;
  savedFlashcardsData: FlashcardsCreateCommand | null;
}

export default function NewCollectionDialog({ onClose, savedFlashcardsData }: NewCollectionDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Jeśli mamy zapisane fiszki, przejdź do dialogu importu
      if (savedFlashcardsData) {
        setTimeout(() => {
          setIsImportDialogOpen(true);
        }, 1500);
      } else {
        // Jeśli nie ma zapisanych fiszek, zamknij dialog po wyświetleniu komunikatu
        setTimeout(() => {
          onClose();
        }, 1500);
      }
    ...
    >
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} data-test-id="new-collection-form">
          {error && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 text-red-600 rounded" data-test-id="error-message">
              {error}
            </div>
          )}
      ...
      {isImportDialogOpen && newCollectionId && savedFlashcardsData && (
        <ImportFlashcardsDialog
          collectionId={newCollectionId}
          onClose={handleFinish}
          onSuccess={handleFinish}
          flashcardsData={savedFlashcardsData}
